...
angular.module('tidbitsApp')
  .controller('Solve3Ctrl', function ($scope, $http) {
    ...
    $scope.xmark = function (b) {
  	...
  	$scope.refresh = function () {
        ...
        $scope.disqualified = res.data.disqualified;
	  		$scope.solvedQuestions = res.data.solved3;
	  		$http.get('/api/questions/next3/').then(function (res) {
          console.log("Hello");
	  		  if(res.data == "") {
	  		  	$(".nq").hide();
	  		  	$(".nnq").show();
    ...
    $scope.refresh();
  	...
  	$scope.subAns = function() {
  	  $http.post('/api/answers/' + $scope.nextQuestionID + '/3', { answer: $scope.answer })
        .then(function (res) {
    	  	if(res.status == 200) {
    	  		$scope.refresh();
